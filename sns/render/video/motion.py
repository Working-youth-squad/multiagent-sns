...
화면 문법 (1080×1920):
  - 배경: 해소된 사진/생성 이미지를 **가로 꽉 채우고** 상하 경계는 같은 이미지의
    블러 커버로 페이드(테두리 없는 전체화면 느낌), 없으면 그라데이션. 느린 줌(zoompan)
...
def _cover(img: Image.Image, width: int, height: int) -> Image.Image:
    """가운데 기준 커버 크롭 — 블러 백드롭이 프레임을 채울 때 쓴다."""
...
# 원본↔블러 경계를 녹이는 페이드 폭(짧은 변 대비) — 딱 떨어지는 사각 테두리가
# "이미지가 박혀 있다"로 읽혔다(실사용 피드백).
_FEATHER_RATIO = 0.14


def _fit_on_blur(img: Image.Image, width: int, height: int) -> Image.Image:
    """이미지를 **가로 꽉 채워** 앉힌다 — 커버 크롭은 정사각 원본의 좌우 44%를 잘라
    피사체가 화면 밖으로 나갔고(실사용 피드백 1), 여백을 남긴 contain은 사각 테두리가
    도드라졌다(실사용 피드백 2). 가로는 프레임과 같아 좌우 잘림·테두리가 없고, 상하
    경계는 같은 이미지의 블러 커버로 페이드시켜 전체화면처럼 읽힌다."""
    # 백드롭을 어둡게 하지 않는다 — 밝기가 다르면 페이드로 이음새를 지워도 밝은
    # 사각형이 도드라진다. 자막 가독성은 자막 밴드가 따로 책임진다.
    backdrop = _cover(img, width, height).filter(ImageFilter.GaussianBlur(radius=40))
    # 가로 폭 기준. 세로가 더 긴 이미지는 프레임을 넘지 않게 contain으로 물러난다.
    scale = min(width / img.width, height / img.height)
    fg = img.resize((round(img.width * scale), round(img.height * scale)), Image.Resampling.LANCZOS)
    feather = max(1, round(min(fg.width, fg.height) * _FEATHER_RATIO))
    mask = Image.new("L", (fg.width, fg.height), 255)
    draw = ImageDraw.Draw(mask)
    for i in range(feather):  # 바깥(0) → 안(feather)으로 갈수록 불투명
        alpha = round(255 * (i + 1) / (feather + 1))
        draw.rectangle((i, i, fg.width - 1 - i, fg.height - 1 - i), outline=alpha)
    backdrop.paste(fg, ((width - fg.width) // 2, (height - fg.height) // 2), mask)
    return backdrop
...
def _bg_png(slide: Slide, spec: VideoSpec, fetch_image: FetchImage | None) -> bytes:
    if slide.image_ref:
        ...
        canvas = _fit_on_blur(photo, spec.width, spec.height)
    else:

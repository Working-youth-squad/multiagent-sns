...
3. media_spec을 set_media_spec에 JSON으로 넘긴다. 틀리면 오류가 돌아오니 고쳐 다시 호출한다.
   ...
   - 릴스/쇼츠(reels/shorts):
     ...
     * image_prompt(선택): 코드도 도해도 아닌 컷에 **그림을 만들어 넣을** 구도 설명
       (영어 한 문장). 화풍(어두운 배경·플랫 벡터·글자 없음)은 코드가 붙이니 **무엇이
       어떻게 놓여 있는지만** 써라: "one glowing cube standing apart from a grey row".
       사진과 달리 원하는 구도를 직접 말할 수 있으니 추상 개념에도 쓸 수 있다.
       글자·숫자가 필요한 그림이면 쓰지 마라 — 생성 글자는 뭉개진다. concept을 써라.
     * code·concept·image_query·image_prompt는 **넷 중 하나만** 쓴다(정사각은 하나다).
       넷 다 마땅치 않으면 비워라 — 억지로 붙인 그림보다 빈 배경이 낫다.

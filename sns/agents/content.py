...
3. media_spec을 set_media_spec에 JSON으로 넘긴다. 틀리면 오류가 돌아오니 고쳐 다시 호출한다.
   ...
   - 릴스/쇼츠(reels/shorts):
     ...
     * image_query(선택): 코드가 **없는** 컷에만. 가운데를 채울 스톡 사진 검색어를
       **영어로** 2~4단어 적는다("tangled network cables", "empty server room").
       코드와 함께 쓰면 거부된다(정사각은 하나다). 사람·얼굴·로고·정치·무기가 들어간
       검색어는 게이트에 막히니 **사물이나 추상 이미지**로 적어라. 마땅한 게 없으면
       비워라 — 억지로 붙인 제네릭 스톡보다 빈 배경이 낫다.

...
| 테이블 | 역할 | 핵심 불변식(CHECK) |
|---|---|---|
| `channel` | 발행 채널(IG/YouTube) + 암호화 토큰 | platform enum, status enum(active/paused/revoked — 발행 중지 표현), 토큰 평문 저장 금지 |
| `cycle` | 발행 사이클(계획 단위) | status enum |
| `topic` | 개발자 주제 풀 | status enum |
| `topic_stats` | 주제×포맷×채널 성과 집계(밴딧 입력) | (topic_id, format, platform) 유일 |
| `content_item` | 생성 콘텐츠(대본/카피/문안 + 렌더 스펙) + **훅 패턴 [v2]** + **카테고리 [v3]** | format·status enum(draft/approved/rejected), **hook_pattern enum(bold_claim/curiosity/story/shock/question — FR-G5)**, **topic_category enum(신기술/기초지식/꿀팁/현직자일상/개발자유머 — FR-G1·[04](04-트렌드조사.md) §4)** |
| `media_asset` | 렌더된 미디어 + checksum + 품질게이트 결과 | kind enum, quality_status enum(passed/failed) — 기본값 `failed`(fail-closed, FR-Q3) |

...
> **처리 결과 (2026-08-26 갱신): 8건 전부 이 PR 안에서 고쳤다.** 최초 방침은 "머지 후
> 후속 브랜치에서 일괄 수정"이었으나, 머지 시점에 R2·R7이 핸드오프가 연동 팀에 약속한
> 계약(§2 토글, §1.2 JSON)에 직접 걸려 있어 **틀린 계약이 먼저 배포되는** 문제가 있었다.
> 성격별로 커밋을 나눠 처리했고, 아래 표의 '처리' 열이 그 대응이다. 각 항목 본문은
> **무엇이 왜 문제였는지의 기록**으로 그대로 둔다 — 회귀 테스트가 가리키는 근거다.
>
> R2의 잔존 한계(정책 결정)는 **③ 별도 토글**을 골랐다: `exclude_ignore_spaces`
> (CLI `--exclude-ignore-spaces`), 기본 off. 오탐 위험과 켤 조건은 핸드오프 §2.1에 적었다.
...
| # | 등급 | 한 줄 | 처리 |
|---|---|---|---|
| [R1](#r1) | medium | 비-`str` 원소를 `str()`로 문자열화 → 깨진 키워드가 LLM까지 흘러감 | ✅ `46f7d2a` |
| [R2](#r2) | medium | 제외 키워드 판정이 공백 변종을 못 잡고, 소스 순서에 따라 결과가 뒤집힘 | ✅ `341df50` |
| [R3](#r3) | medium | `service=` 주입이 무력화 — 하드코딩 3소스가 강제됨 | ✅ `9a23817` |
| [R4](#r4) | low | 중복 소스명이 두 번 계산 → 관측 1건이 `present_count=2`·`rank_std=0.0` | ✅ `9a23817` |
| [R5](#r5) | low | 자기제외에 **에코된** 질의어를 씀 → 질의어 자신이 후보로 남을 수 있음 | ✅ `46f7d2a` |
| [R6](#r6) | low | 공백 질의어가 트레이스백 + exit 1 (문서가 약속한 exit 2 아님) | ✅ `6c02d0d` |
| [R7](#r7) | low | `ranking_to_dict` 계약 구멍 — `pool` 누락 · `unscored` 이중 집계 | ✅ `3b8b771` |
| [R8](#r8) | low | `--no-band` 힌트가 밴드를 안 쓴 경우에도 무조건 출력 | ✅ `6c02d0d` |
| 별건 | — | `USER_AGENT` 리터럴 세 번째 복사본 | ✅ `93cfb08` |

여덟 건 모두 회귀 테스트를 함께 넣었다 — 표의 커밋이 그 테스트를 들고 있다.

**연동 팀 영향(해소됨)**: R2·R7은 핸드오프 문서
(`docs/handoff/2026-08-25-keyword-ranking-handoff.md`)가 챗봇/LLM 팀에게 약속한
계약(§2 토글, §1.2 JSON)에 직접 걸렸다. 수정과 **같은 커밋에서** 그 문서를 갱신했으므로
지금은 핸드오프가 코드와 일치한다.
...
성격이 달라 커밋을 나눴다. 아래 번호가 실제 커밋 순서다.
...
3번의 정책 선택(R2 잔존 한계)은 **별도 토글**로 갔다 — 기본 동작을 바꾸지 않고 호출자에게
선택권을 남긴다. 어느 쪽이든 오탐과 미탐을 맞바꾸는 문제라, 도메인이 부정 키워드 목록을
얼마나 좁게 통제하는지에 따라 답이 달라진다(핸드오프 §2.1).

덤으로 이 문서 자체가 CI를 빨갛게 만들고 있었다 — 코드블록이 `ruff format` 규칙과 어긋나
`ruff format --check .`가 exit 1이었다(`05305dd`).

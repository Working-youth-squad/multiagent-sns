...
- **Phase 1 — M1(실환경 관통)**: IG 피드/릴스 + YT 쇼츠 각 1건 자동 발행 → 지표 → reward → 리포트까지 한 번 관통(수동 트리거 허용). **발행 전 품질 게이트(FR-Q) 통과 필수.**
- **Phase 2 — 자율 루프**: 스케줄러 상주 + 밴딧 학습 + 플레이북 반영 + 분석글. **비용 상한(cost cap) 초기부터 적용**(콜드스타트 탐색기 호출 폭증 방지).
...
| **자율 루프 비용 폭증(콜드스타트 탐색기 LLM/TTS 호출 급증)** | **사이클·일 단위 비용 상한(cost cap)을 Phase 2부터 적용**, 초과 시 발행 스킵+알림 |

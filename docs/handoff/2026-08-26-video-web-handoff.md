...
**상태**: 코드 완료 — **PR #44**(커밋 4개, CI 통과). 챗봇(:8003)에서 대화로 포맷·제작
방식을 고르고, 영상이 초안 카드에 재생기로 뜨고, mp4로 내려받히고, 유튜브 실 업로드
배선이 붙어 있다.
...
- **워크트리가 9개다** (`git worktree list`) — `cd`만 바꾸면 도구가 다른 워크트리를 본다.
- **heredoc으로 파이썬 파일을 쓰지 말 것** — Git Bash에서 `
`이 실제 개행으로 망가져
  문자열 리터럴이 깨진다(SyntaxError). 편집 도구를 쓸 것.
